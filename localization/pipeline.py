...
class LocalizationPipeline:
    ...
    def run(
        ...
        logger.info("Extracting features from UAV and map images")
        ...
        logger.info("Extracted %d features from UAV, %d from map", len(uav_keypoints), len(map_keypoints))
        ...
        if len(matches) < self.model.min_points:
             logger.error("Insufficient matches to estimate transformation (%d found, %d required)", 
                          len(matches), self.model.min_points)
        ...
        logger.info("RANSAC finished. Iterations: %d, Inliers: %d", iterations_done, n_inliers)
